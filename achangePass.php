            ...
            <div class="center">
                <?php
                    if(isset($_POST['submit'])){
                        $sql="SELECT * FROM admin WHERE ADMIN_PASS='{$_POST["opass"]}' and ADMIN_ID = '{$_SESSION["ADMIN_ID"]}'";
                        $res = $db->query($sql);
                        if($res->num_rows>0){
                            $s ="update admin set ADMIN_PASS='{$_POST["npass"]}' WHERE ADMIN_ID=".$_SESSION["ADMIN_ID"];
                            $db->query($s);
