            ...
            <div class="center">
                <?php
                    if(isset($_POST['submit'])){
                        $sql="SELECT * FROM student WHERE STUDENT_PASS='{$_POST["opass"]}' and STUDENT_ID = '{$_SESSION["STUDENT_ID"]}'";
                        $res = $db->query($sql);
                        if($res->num_rows>0){
                            $s ="update student set STUDENT_PASS='{$_POST["npass"]}' WHERE STUDENT_ID=".$_SESSION["STUDENT_ID"];
                            $db->query($s);
